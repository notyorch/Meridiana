...
def mustache_to_jinja(template_content):
    ...
    # {{#features}} ... {{/features}} -> {% for feature in features %} ... {% endfor %}
    template_content = re.sub(r'\{\{#features\}\}(.*?)\{\{/features\}\}', 
                              r'{% for feature in features %}\1{% endfor %}', 
                              template_content, flags=re.DOTALL)
    
    # Inside the loop, replace {{feature_name}} with {{feature.feature_name}}
    # We only want to replace it where it was inside the features loop. 
    # Since we already converted the loop tags, we can look for it between {% for feature in features %} and {% endfor %}
    # But for simplicity, if it's the only one, a global replace is fine.
...
def generate_brochures():
    ...
    # Filter for P001-P010
    target_ids = [f'P{str(i).zfill(3)}' for i in range(1, 11)]
    df_prop = df_prop[df_prop['property_id'].isin(target_ids)]
    
    for _, prop in df_prop.iterrows():
        ...
        # Get Agent
        agent = df_agents[df_agents['agent_id'] == prop['agent_id']].iloc[0]
